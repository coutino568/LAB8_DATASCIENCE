...
dec_imp_Diesel$Date = time(importacion_Diesel)

dec_cons_super <- decompose(Consumo_super)
dec_cons_super$Date <- time(Consumo_super)
dec_imp_super <- decompose(importacion_super)
dec_imp_super$Date <- time(importacion_super)

dec_cons_regular <- decompose(Consumo_regular)
dec_cons_regular$Date <- time(Consumo_regular)
dec_imp_regular <- decompose(importacion_regular)
dec_imp_regular$Date <- time(importacion_regular)
...
server <- function(input, output) {
  ...
  output$BarPlot <- renderPlotly({
        ...
        Diesel_desc <- if (IsConsumo) dec_cons_Diesel else dec_imp_Diesel
        Diesel_desc <- window(Diesel_desc$x, monthFactor, monthFactor)
        Regular_desc <- if (IsConsumo) dec_cons_regular else dec_imp_regular
        Regular_desc <- window(Regular_desc$x, monthFactor, monthFactor)
        Super_desc <- if (IsConsumo) dec_cons_super else dec_imp_super
        Super_desc <- window(Super_desc$x, monthFactor, monthFactor)
        
        
        fig <- plot_ly(
          x = c("Diesel", "Regular", "Super"),
          y = c(Diesel_desc, Regular_desc, Super_desc),
          name = if (IsConsumo) "Consumo de combustibles" else "Importación de combustibles",
          type = "bar")
