...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("COSNUMO DE COMBUSTIBLES"),
...
server <- function(input, output) {
  ...
  output$SamplePlot <- renderPlot({
    min <- input$bans
    max <- input$bans
    rango <- dec_cons_Diesel[dec_cons_Diesel$Date > min & dec_cons_Diesel$Date < max]
...
```
```{r}
rango <- dec_cons_Diesel[dec_cons_Diesel$Date >"2000-1-1" & dec_cons_Diesel$Date < "2010-1-1"]
rango
```
